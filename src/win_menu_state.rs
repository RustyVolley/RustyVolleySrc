...
use state_manager::{RustyGameState::*, StateTransition::*, *};
            ...
            cloned_font_ref.borrow_mut().execute(|a_font| {
                if self.home_menu_text.is_none() {
                    let home_menu_text = match self.winning_player {
                        LeftPlayer => a_font
                            .render(&format!("Player 1 won!"), &game_assets.font_style)
                            .unwrap(),
                        RightPlayer => a_font
                            .render(&format!("Player 2 won!"), &game_assets.font_style)
                            .unwrap(),
                        _ => a_font
                            .render(&format!("Unknown Player won!"), &game_assets.font_style)
                            .unwrap(),
                    };
    ...
    fn handle_event(&mut self, event: &Event, _window: &mut Window) -> StateTransition {
        let transition = match *event {
            Event::Key(Key::Space, ButtonState::Pressed) => {
                StateTransition::StateLessTransition(NewGameMenu)
            }
