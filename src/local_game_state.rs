...
impl LocalGameState {
    ...
    pub fn step(&mut self, game_assets: &mut GamesAssets) -> StateTransition {
        ...
        if self.use_bot_right {
            ...
            self.duel_match
                .get_world()
                .set_player_input(RightPlayer, self.bot_right.compute_input());
        ...
        if self.use_bot_left {
            ...
            self.duel_match
                .get_world()
                .set_player_input(LeftPlayer, self.bot_left.compute_input());
        ...
        if self.frame_events.iter().any(|x| {
            *x == FrameEvent::EventError(LeftPlayer) || *x == FrameEvent::EventError(RightPlayer)
        }) {
        ...
        if self
            .frame_events
            .iter()
            .any(|x| *x == FrameEvent::EventWin(LeftPlayer))
        {
            StateTransition::WinStateTransition(LeftPlayer)
        } else if self
            .frame_events
            .iter()
            .any(|x| *x == FrameEvent::EventWin(RightPlayer))
        {
    ...
    pub fn draw_window_content(
        ...
        // draw left player
        {
            ...
            let blob_state = (self.duel_match.get_world().get_blob_state(LeftPlayer) as usize)
                % (BLOBBY_ANIMATION_FRAMES);
        ...
        // draw right player
        {
            ...
            let blob_state = (self.duel_match.get_world().get_blob_state(RightPlayer) as usize)
                % (BLOBBY_ANIMATION_FRAMES);
            let transform = Transform::scale(Vector::new(
                DISPLAY_SCALE_FACTOR * 2.4f32 * 0.5f32,
                DISPLAY_SCALE_FACTOR * 2.4f32 * 0.5f32,
        ...
        // draw the ball
        {
            ...
            let transform =
                Transform::scale(Vector::new(
                    DISPLAY_SCALE_FACTOR * 2.4f32 * 0.5f32,
                    DISPLAY_SCALE_FACTOR * 2.4f32 * 0.5f32,
                )) * Transform::rotate(ball_rot as f32 / std::f32::consts::PI * 180.0f32);
        ...
        // draw the score
        {
            ...
            let should_recreate_texture = self.scoring.score1 != score1
            ...
            cloned_font_ref.borrow_mut().execute(|a_font| {
                if should_recreate_texture {
                    let score1_texture = a_font
                        .render(&format!("{:02}", score1), &game_assets.font_style)
                        .unwrap();

                    self.scoring.score1 = score1;
                    self.scoring.score1_texture = Some(score1_texture);

                    let score2_texture = a_font
                        .render(&format!("{:02}", score2), &game_assets.font_style)
                        .unwrap();
    ...
    pub fn handle_event(&mut self, event: &Event, _window: &mut Window) -> StateTransition {
        ...
        if let &Event::Key(key, state) = event {
            ...
            self.duel_match
                .get_world()
                .set_player_input(LeftPlayer, player_left_input);
            self.duel_match
                .get_world()
                .set_player_input(RightPlayer, player_right_input);
        }
        NoTransition
    }
}
